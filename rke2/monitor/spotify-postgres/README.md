...
| `migration.yaml` | Idempotent DDL in a ConfigMap + a one-shot Job that applies it and creates the `spotify_app` / `grafana_ro` roles. |
...
kubectl -n monitor wait --for=condition=complete job/spotify-postgres-migrate --timeout=180s
