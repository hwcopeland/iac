...
(function (MinimizeRmsd) {
    ...
    function compute(data, result, state) {
        result !== null && result !== void 0 ? result : (result = { bTransform: Mat4.zero(), rmsd: 0.0, nAlignedElements: 0 });
        if (state) {
...
export class RmsdTransformState {
    constructor(data, into) {
        ...
        if (data.centerA)
            ...
            this.centerA = data.centerA = computeCenter(data.a, Vec3(), this.length);
        if (data.centerB)
            this.centerB = data.centerB;
        else
            this.centerB = data.centerB = computeCenter(data.b, Vec3(), this.length);
        this.result = into;
    }
}
