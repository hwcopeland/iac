...
const param_definition_1 = require("./param-definition.js");
...
(function (Clip) {
    ...
    Clip.Params = {
        ...
        objects: param_definition_1.ParamDefinition.ObjectList({
            ...
            position: param_definition_1.ParamDefinition.Vec3((0, vec3_1.Vec3)()),
            rotation: param_definition_1.ParamDefinition.Group({
                axis: param_definition_1.ParamDefinition.Vec3(vec3_1.Vec3.create(1, 0, 0)),
                angle: param_definition_1.ParamDefinition.Numeric(0, { min: -180, max: 180, step: 1 }, { description: 'Angle in Degrees' }),
            }, { isExpanded: true }),
            scale: param_definition_1.ParamDefinition.Vec3(vec3_1.Vec3.create(1, 1, 1)),
            transform: param_definition_1.ParamDefinition.Mat4(mat4_1.Mat4.identity()),
    ...
    const qA = (0, quat_1.Quat)();
    const qB = (0, quat_1.Quat)();
    const vA = (0, vec3_1.Vec3)();
    const vB = (0, vec3_1.Vec3)();
    const mA = (0, mat4_1.Mat4)();
    const mB = (0, mat4_1.Mat4)();
    function getClip(props, clip) {
        ...
        for (let i = 0; i < count; ++i) {
            ...
            invert[i] = p.invert;
            vec3_1.Vec3.toArray(p.position, position, i * 3);
            vec3_1.Vec3.normalize(vA, p.rotation.axis);
            quat_1.Quat.toArray(quat_1.Quat.setAxisAngle(qA, vA, (0, misc_1.degToRad)(p.rotation.angle)), rotation, i * 4);
            vec3_1.Vec3.toArray(p.scale, scale, i * 3);
            mat4_1.Mat4.toArray(p.transform, transform, i * 16);
        }
        return {
    ...
    function areEqual(cA, cB) {
        ...
        for (let i = 0, il = oA.count; i < il; ++i) {
            ...
            if (oA.type[i] !== oB.type[i])
                return false;
            vec3_1.Vec3.fromArray(vA, oA.position, i * 3);
            vec3_1.Vec3.fromArray(vB, oB.position, i * 3);
            if (!vec3_1.Vec3.equals(vA, vB))
                return false;
            vec3_1.Vec3.fromArray(vA, oA.scale, i * 3);
            vec3_1.Vec3.fromArray(vB, oB.scale, i * 3);
            if (!vec3_1.Vec3.equals(vA, vB))
                return false;
            quat_1.Quat.fromArray(qA, oA.rotation, i * 4);
            quat_1.Quat.fromArray(qB, oB.rotation, i * 4);
            if (!quat_1.Quat.equals(qA, qB))
                return false;
            mat4_1.Mat4.fromArray(mA, oA.transform, i * 16);
            mat4_1.Mat4.fromArray(mB, oB.transform, i * 16);
            if (!mat4_1.Mat4.areEqual(mA, mB, common_1.EPSILON))
                return false;
        }
        return true;
    }
