...
(function (StructureSymmetry) {
    ...
    function computeChildAwareTransformGroups(s) {
        ...
        const childGroupIndex = new Map();
        for (let i = 0; i < childGroups.length; i++) {
            for (const u of childGroups[i].units) {
                childGroupIndex.set(u.id, i);
            }
        }
        const groups = (0, util_1.EquivalenceClasses)((u) => { var _a; return (0, hash_functions_1.hash2)(unit_1.Unit.hashUnit(u), (_a = childGroupIndex.get(u.id)) !== null && _a !== void 0 ? _a : -1); }, (a, b) => areUnitsEquivalent(a, b) && childGroupIndex.get(a.id) === childGroupIndex.get(b.id));
        for (const u of s.units)
            groups.add(u.id, u);
        const ret = [];
        for (const eqUnits of groups.groups) {
    ...
    StructureSymmetry.computeChildAwareTransformGroups = computeChildAwareTransformGroups;
    ...
    function areTransformGroupsEquivalent(a, b) {
        ...
        for (let i = 0, il = a.length; i < il; ++i) {
            ...
            if (!int_1.SortedArray.areEqual(a[i].elements, b[i].elements))
                return false;
        }
        return true;
    }
