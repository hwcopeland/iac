...
const isosurface_1 = require("../../mol-gl/compute/marching-cubes/isosurface.js");
...
exports.VolumeIsosurfaceParams = {
    ...
    wrap: param_definition_1.ParamDefinition.Select('auto', param_definition_1.ParamDefinition.arrayToOptions(['off', 'on', 'auto'])),
    floodfill: param_definition_1.ParamDefinition.Select('off', param_definition_1.ParamDefinition.arrayToOptions(['off', 'inside', 'outside']), { description: 'If and how to floodfill the volume. Note that this disables GPU support.' }),
};
exports.VolumeIsosurfaceTextureParams = {
    ...exports.VolumeIsosurfaceParams,
    tryUseGpu: param_definition_1.ParamDefinition.Boolean(true, { hideIf: p => p.floodfill !== 'off' }),
    gpuDataType: param_definition_1.ParamDefinition.Select('byte', param_definition_1.ParamDefinition.arrayToOptions(['byte', 'float', 'halfFloat']), { hideIf: p => !p.tryUseGpu || p.floodfill !== 'off' }),
};
...
function shouldWrap(volume, wrap) {
    if (wrap === 'on')
        return true;
    if (wrap === 'off')
        return false;
    return volume_1.Volume.isPeriodic(volume);
}
const Padding = 1;
...
async function createVolumeIsosurfaceMesh(ctx, volume, key, theme, props, mesh) {
    ...
    const isoLevel = volume_1.Volume.IsoValue.toAbsolute(props.isoValue, volume.grid.stats).absoluteValue;
    let scalarField = volume.grid.cells;
    if (props.floodfill !== 'off') {
        scalarField = linear_algebra_1.Tensor.createFloodfilled(scalarField, isoLevel, props.floodfill);
    }
    if (shouldWrap(volume, props.wrap)) {
        scalarField = (0, util_1.createWrappedTensor)(scalarField);
    }
    ...
    const surface = await (0, algorithm_1.computeMarchingCubesMesh)({
        ...
        idField: linear_algebra_1.Tensor.create(scalarField.space, linear_algebra_1.Tensor.Data1(ids))
...
function IsosurfaceMeshVisual(materialId) {
    return (0, visual_1.VolumeVisual)({
        ...
        setUpdateState: (state, newVolume, currentVolume, newProps, currentProps) => {
            state.createGeometry = (!volume_1.Volume.IsoValue.areSame(newProps.isoValue, currentProps.isoValue, newVolume.grid.stats) ||
...
(function (VolumeIsosurfaceTexture) {
    ...
    function get(volume, webgl, props) {
        ...
        const gridDataDim = linear_algebra_1.Vec3.subScalar((0, linear_algebra_1.Vec3)(), gridDimension, wrap ? 1 : 0);
        ...
        if (((_a = volume._propertyData[name]) === null || _a === void 0 ? void 0 : _a.dataType) !== dataType || ((_b = volume._propertyData[name]) === null || _b === void 0 ? void 0 : _b.wrap) !== wrap) {
...
function createVolumeIsosurfaceTextureMesh(ctx, volume, key, theme, props, textureMesh) {
    ...
    if (shouldWrap(volume, props.wrap)) {
        volume = (0, util_1.createWrappedVolume)(volume);
    }
    ...
    const boundingSphere = volume_1.Grid.getBoundingSphere(volume.grid); // getting isosurface bounding-sphere is too expensive here
    const create = (textureMesh) => {
        ...
        const gv = (0, isosurface_1.extractIsosurface)(webgl, texture, gridDimension, gridTexDim, gridDataDim, gridTexScale, transform, isoLevel, value < 0, false, axisOrder, true, buffer === null || buffer === void 0 ? void 0 : buffer.vertex, buffer === null || buffer === void 0 ? void 0 : buffer.group, buffer === null || buffer === void 0 ? void 0 : buffer.normal);
