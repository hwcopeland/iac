...
(function (Volume) {
    ...
    function getBoundingSphere(volume, boundingSphere) {
        const gs = grid_1.Grid.getBoundingSphere(volume.grid);
        if (!boundingSphere)
            boundingSphere = (0, geometry_1.Sphere3D)();
        if (volume.instances.length === 0)
            return geometry_1.Sphere3D.copy(boundingSphere, gs);
        const spheres = [];
        for (let i = 0, il = volume.instances.length; i < il; ++i) {
            const { transform } = volume.instances[i];
            spheres.push(geometry_1.Sphere3D.transform((0, geometry_1.Sphere3D)(), gs, transform));
        }
    ...
    function getPeriodicRange(volume) {
        ...
        if (!v._localPropertyData['__periodicRange__']) {
            const dim = linear_algebra_1.Vec3.fromArray((0, linear_algebra_1.Vec3)(), v.grid.cells.space.dimensions, 0);
            const gridToCartn = grid_1.Grid.getGridToCartesianTransform(v.grid);
            linear_algebra_1.Vec3.transformMat4(dim, dim, gridToCartn);
            const min = linear_algebra_1.Vec3.create(Infinity, Infinity, Infinity);
            const max = linear_algebra_1.Vec3.create(-Infinity, -Infinity, -Infinity);
            const tg = (0, linear_algebra_1.Vec3)();
            for (const { transform } of v.instances) {
                linear_algebra_1.Mat4.getTranslation(tg, transform);
                linear_algebra_1.Vec3.div(tg, tg, dim);
                linear_algebra_1.Vec3.round(tg, tg);
                linear_algebra_1.Vec3.min(min, min, tg);
                linear_algebra_1.Vec3.max(max, max, tg);
            }
            linear_algebra_1.Vec3.addScalar(max, max, 1);
            v._localPropertyData['__periodicRange__'] = { min, max };
        }
        return v._localPropertyData['__periodicRange__'];
    }
    Volume.getPeriodicRange = getPeriodicRange;
    ;
    function getPeriodicMapping(volume) {
        ...
        if (!v._localPropertyData['__periodicInstanceMapping__']) {
            ...
            for (let i = 0, il = v.instances.length; i < il; i++) {
                ...
                linear_algebra_1.Vec3.div(tg, tg, dim);
                linear_algebra_1.Vec3.round(tg, tg);
                linear_algebra_1.Vec3.sub(tg, tg, min);
                map.set((0, hash_functions_1.mortonOrder3d)(tg[0], tg[1], tg[2]), i);
            }
            const dd = linear_algebra_1.Vec3.fromArray((0, linear_algebra_1.Vec3)(), v.grid.cells.space.dimensions, 0);
            v._localPropertyData['__periodicInstanceMapping__'] = {
                get(x, y, z) {
                    linear_algebra_1.Vec3.set(tg, x, y, z);
                    linear_algebra_1.Vec3.div(tg, tg, dd);
                    linear_algebra_1.Vec3.floor(tg, tg);
                    const instance = map.get((0, hash_functions_1.mortonOrder3d)(tg[0], tg[1], tg[2]));
                    if (instance === undefined)
                        return;
                    linear_algebra_1.Vec3.set(tg, x, y, z);
                    linear_algebra_1.Vec3.mod(tg, tg, dd);
                    const cell = v.grid.cells.space.dataOffset(tg[0], tg[1], tg[2]);
    ...
    Volume.getPeriodicMapping = getPeriodicMapping;
    ;
    function isPeriodic(volume) {
        return volume.grid.periodicity === 'xyz';
    }
    Volume.isPeriodic = isPeriodic;
