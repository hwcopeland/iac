...
exports.TMAlign = void 0;
const mat4_1 = require("./mat4.js");
const vec3_1 = require("./vec3.js");
const minimize_rmsd_1 = require("./minimize-rmsd.js");
const interpolate_1 = require("../../interpolate.js");
var TMAlign;
(function (TMAlign) {
    TMAlign.Positions = minimize_rmsd_1.MinimizeRmsd.Positions;
    /**
    ...
    function compute(input) {
        ...
        const d0Search = (0, interpolate_1.clamp)(d0, 4.5, 8);
        ...
        return swap
            ? {
                bTransform: mat4_1.Mat4.invert(transform, transform),
                tmScoreA: tmScoreB,
                tmScoreB: tmScoreA,
                rmsd,
    ...
    function calculateTMScore(xa, ya, alignA, alignB, transform, d0, normLength) {
        ...
        for (let i = 0; i < alignA.length; i++) {
            const ai = alignA[i];
            const bi = alignB[i];
            vec3_1.Vec3.transformMat4(v, ya[bi], transform);
            const distSq = vec3_1.Vec3.squaredDistance(xa[ai], v);
    ...
    function calculateRMSD(xa, ya, alignA, alignB, transform) {
        ...
        const v = (0, vec3_1.Vec3)();
        for (let i = 0; i < alignA.length; i++) {
            vec3_1.Vec3.transformMat4(v, ya[alignB[i]], transform);
            sumSq += vec3_1.Vec3.squaredDistance(xa[alignA[i]], v);
        }
...
})(TMAlign || (exports.TMAlign = TMAlign = {}));
/**
...
class TMAlignState {
    constructor(xa, ya, lenA, lenB, d0, d0Search, scoreD8) {
        ...
        this.bestAlignmentA = [];
        this.bestAlignmentB = [];
        this.bestTransform = mat4_1.Mat4.identity();
        this.xa = xa;
        this.ya = ya;
        this.lenA = lenA;
        this.lenB = lenB;
        ...
        this.posA = minimize_rmsd_1.MinimizeRmsd.Positions.empty(lenA);
        this.posB = minimize_rmsd_1.MinimizeRmsd.Positions.empty(lenB);
        // Initialize cached objects for MinimizeRmsd operations
        this.rmsdResult = {
            bTransform: mat4_1.Mat4.zero(),
            rmsd: 0,
            nAlignedElements: 0
        };
        // Initialize with dummy positions - will be reset on first use
        this.rmsdState = new minimize_rmsd_1.RmsdTransformState({ a: this.posA, b: this.posB, length: 0 }, this.rmsdResult);
    ...
    optimizeFinalAlignment() {
        ...
        for (const cutoff of cutoffs) {
            ...
            for (let i = 0; i < this.bestAlignmentA.length; i++) {
                ...
                vec3_1.Vec3.transformMat4(v, ya[bi], this.bestTransform);
                const distSq = vec3_1.Vec3.squaredDistance(v, xa[ai]);
                if (distSq <= cutoffSq) {
                    filteredA[n] = ai;
                    filteredB[n] = bi;
                    n++;
            ...
            if (score > this.bestScore) {
                ...
                mat4_1.Mat4.copy(this.bestTransform, transform);
            }
        }
    ...
    refineWithCutoff(cutoff) {
        ...
        for (let iter = 0; iter < 10; iter++) {
            ...
            for (let i = 0; i < this.lenB; i++) {
                vec3_1.Vec3.transformMat4(yt[i], ya[i], transform);
            }
            ...
            if (newScore > this.bestScore) {
                ...
                mat4_1.Mat4.copy(this.bestTransform, newTransform);
            }
            else {
                break;
            }
    ...
    getInitialAlignment() {
        ...
        if (this.bestAlignmentA.length >= 3) {
            mat4_1.Mat4.copy(this.bestTransform, this.kabsch(this.bestAlignmentA, this.bestAlignmentB));
    ...
    extendFromSeed(initialTransform) {
        ...
        for (let iter = 0; iter < 20; iter++) {
            ...
            for (let i = 0; i < lenB; i++) {
                vec3_1.Vec3.transformMat4(yt[i], ya[i], currentTransform);
            }
    ...
    trimmedKabsch(alignA, alignB, currentTransform, cutoff) {
        ...
        for (let i = 0; i < alignA.length; i++) {
            const ai = alignA[i];
            const bi = alignB[i];
            vec3_1.Vec3.transformMat4(v, ya[bi], currentTransform);
            const distSq = vec3_1.Vec3.squaredDistance(v, xa[ai]);
            if (distSq <= cutoffSq) {
    ...
    trimmedKabschWithTransformedCoordinates(alignA, alignB, yt, cutoff) {
        ...
        for (let i = 0; i < alignA.length; i++) {
            const ai = alignA[i];
            const bi = alignB[i];
            const distSq = vec3_1.Vec3.squaredDistance(yt[bi], xa[ai]);
            if (distSq <= cutoffSq) {
                trimmedAlignA.push(ai);
                trimmedAlignB.push(bi);
            }
    ...
    nwdpStructure(xa, yt, d02, gapOpen) {
        ...
        for (let i = 1; i <= lenA; i++) {
            for (let j = 1; j <= lenB; j++) {
                const distSq = vec3_1.Vec3.squaredDistance(xa[i - 1], yt[j - 1]);
    ...
    kabsch(alignA, alignB) {
        ...
        minimize_rmsd_1.MinimizeRmsd.compute({ a: posA, b: posB, length: n }, rmsdResult, rmsdState);
