...
exports.createVolumeSliceCellLocationIterator = createVolumeSliceCellLocationIterator;
...
exports.createWrappedTensor = createWrappedTensor;
exports.createWrappedVolume = createWrappedVolume;
exports.createPeriodicVolume = createPeriodicVolume;
...
const number_conversion_1 = require("../../mol-util/number-conversion.js");
...
function eachVolumeLoci(loci, volume, props, apply) {
    ...
    if (volume_1.Volume.isLoci(loci)) {
        ...
        if (interval_1.Interval.is(loci.instances)) {
            const start = interval_1.Interval.start(loci.instances) * cellCount;
            ...
            for (let i = 0, il = loci.instances.length; i < il; ++i) {
                ...
                if (apply(interval_1.Interval.ofBounds(offset, offset + cellCount)))
                    changed = true;
            }
        }
    }
        ...
        if (props === null || props === void 0 ? void 0 : props.isoValue) {
            ...
            for (let i = 0, il = data.length; i < il; ++i) {
                if ((0, common_1.equalEps)(v, data[i], eps)) {
                    ordered_set_1.OrderedSet.forEach(loci.instances, j => {
                        const offset = j * cellCount;
                        if (apply(interval_1.Interval.ofSingleton(offset + i)))
                            changed = true;
                    });
                }
            }
        ...
        if (props === null || props === void 0 ? void 0 : props.segments) {
            for (const { segments, instances } of loci.elements) {
                if (ordered_set_1.OrderedSet.areIntersecting(segments, props.segments)) {
                    ordered_set_1.OrderedSet.forEach(instances, j => {
                        const offset = j * cellCount;
                        if (apply(interval_1.Interval.ofBounds(offset, offset + cellCount)))
                            changed = true;
            ...
            if (segmentation) {
                const set = new Set();
                for (const { segments, instances } of loci.elements) {
                    for (let i = 0, _i = ordered_set_1.OrderedSet.size(segments); i < _i; i++) {
                        const o = ordered_set_1.OrderedSet.getAt(segments, i);
                        set_1.SetUtils.add(set, segmentation.segments.get(o));
                    }
                    ...
                    for (let i = 0, il = d.length; i < il; ++i) {
                        if (s.includes(d[i])) {
                            for (let j = 0, _j = ordered_set_1.OrderedSet.size(instances); j < _j; j++) {
                                const offset = j * cellCount;
                                if (apply(interval_1.Interval.ofSingleton(i + offset)))
                                    changed = true;
                            }
                        }
                    }
...
function createVolumeCellLocationIterator(volume, key) {
...
function createVolumeSliceCellLocationIterator(volume, key, image) {
    const mapping = image.meta.mapping;
    if (!mapping)
        return createVolumeCellLocationIterator(volume, key);
    const groupCount = mapping.cell.length;
    const instanceCount = volume_1.Volume.isPeriodic(volume) ? 1 : volume.instances.length;
    const location = volume_1.Volume.Cell.Location(volume);
    const getLocation = (groupIndex, instanceIndex) => {
        location.cell = groupIndex;
        location.instance = instanceIndex;
    ...
    const nonInstanceable = volume_1.Volume.isPeriodic(volume);
    return (0, location_iterator_1.LocationIterator)(groupCount, instanceCount, 1, getLocation, nonInstanceable);
}
//
 ...
 */
function createWrappedTensor(tensor) {
    const { space, data } = tensor;
    const { get, set, add, dataOffset } = space;
    const [xn, yn, zn] = space.dimensions;
    const _dimensions = vec3_1.Vec3.create(xn + 1, yn + 1, zn + 1);
    const _get = (data, x, y, z) => get(data, x % xn, y % yn, z % zn);
    const _set = (data, x, y, z, d) => set(data, x % xn, y % yn, z % zn, d);
    const _add = (data, x, y, z, d) => add(data, x % xn, y % yn, z % zn, d);
    const _dataOffset = (x, y, z) => dataOffset(x % xn, y % yn, z % zn);
    const _space = {
        ...space,
 ...
 * Only intended for isosurface construction.
 */
function createWrappedVolume(volume) {
    const { grid } = volume;
    const matrix = volume_1.Grid.getGridToCartesianTransform(grid);
    const _grid = {
        ...grid,
...
function createPeriodicVolume(volume, min, max) {
    const { grid } = volume;
    const { space } = grid.cells;
    const { get, set, add, dataOffset } = space;
    const [xn, yn, zn] = space.dimensions;
    const r = vec3_1.Vec3.sub((0, vec3_1.Vec3)(), max, min);
    const _dimensions = vec3_1.Vec3.create(xn * r[0], yn * r[1], zn * r[2]);
    const _get = (data, x, y, z) => get(data, x % xn, y % yn, z % zn);
    const _set = (data, x, y, z, d) => set(data, x % xn, y % yn, z % zn, d);
    const _add = (data, x, y, z, d) => add(data, x % xn, y % yn, z % zn, d);
    const _dataOffset = (x, y, z) => dataOffset(x % xn, y % yn, z % zn);
    ...
    const matrix = volume_1.Grid.getGridToCartesianTransform(volume.grid);
    const t = mat4_1.Mat4.getScaling((0, vec3_1.Vec3)(), matrix);
    vec3_1.Vec3.mul(t, t, space.dimensions);
    const s = vec3_1.Vec3.mul((0, vec3_1.Vec3)(), min, t);
    mat4_1.Mat4.setTranslation(matrix, s);
    const _transform = { kind: 'matrix', matrix };
    const _grid = {
        ...grid,
    ...
    return {
        ...
        grid: _grid,
        parent: volume,
        instances: [{ transform: mat4_1.Mat4.identity() }],
    };
}
