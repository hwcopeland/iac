...
exports.RmsdTransformState = exports.MinimizeRmsd = void 0;
const mat4_1 = require("./mat4.js");
const vec3_1 = require("./vec3.js");
const evd_1 = require("../matrix/evd.js");
const matrix_1 = require("../matrix/matrix.js");
var MinimizeRmsd;
(function (MinimizeRmsd) {
    ...
    function compute(data, result, state) {
        result !== null && result !== void 0 ? result : (result = { bTransform: mat4_1.Mat4.zero(), rmsd: 0.0, nAlignedElements: 0 });
        if (state) {
...
class RmsdTransformState {
    constructor(data, into) {
        ...
        this.a = data.a;
        this.b = data.b;
        this.length = (_a = data.length) !== null && _a !== void 0 ? _a : data.a.x.length;
        if (data.centerA)
            this.centerA = data.centerA;
        else
            this.centerA = data.centerA = computeCenter(data.a, (0, vec3_1.Vec3)(), this.length);
        if (data.centerB)
            this.centerB = data.centerB;
        else
            this.centerB = data.centerB = computeCenter(data.b, (0, vec3_1.Vec3)(), this.length);
        this.result = into;
    }
}
exports.RmsdTransformState = RmsdTransformState;
// evdCache, translateB, rotateB, tempMatrix are overwritten.
function resetRmsdState(state, data, into) {
    var _a;
    state.a = data.a;
    state.b = data.b;
    state.length = (_a = data.length) !== null && _a !== void 0 ? _a : data.a.x.length;
    if (data.centerA)
        state.centerA = data.centerA;
    else
        data.centerA = computeCenter(data.a, state.centerA, state.length);
    if (data.centerB)
        state.centerB = data.centerB;
    else
        data.centerB = computeCenter(data.b, state.centerB, state.length);
    state.result = into;
}
function computeCenter(pos, toCenter, L) {
    ...
    vec3_1.Vec3.set(toCenter, xSum / L, ySum / L, zSum / L);
    return toCenter;
}
