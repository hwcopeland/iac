    ...
    return {
        type: {
            ...
            params: {
                ...
                interior: {
                    color: color_1.Color.fromNormalizedRgb(0, 0, 0),
                    colorStrength: 0.15,
...
async function createMmcifHierarchy(plugin, trajectory) {
    ...
    const coarseGrained = model_1.Model.isCoarseGrained(model.data);
    ...
    const base = await state.build()
        .to(model)
        .apply(model_2.MmcifAssembly, { id: '' })
        .commit();
    ...
    const entRoot = await state.build()
        .toRoot()
        .apply(state_1.MesoscaleGroup, { ...groupParams, root: true, index: -1, tag: 'ent:', label: 'entity', color: { type: 'custom', illustrative: false, value: names_1.ColorNames.white, variability: 20, shift: 0, lightness: 0, alpha: 1, emissive: 0 } }, { tags: 'group:ent:', state: { isCollapsed: false, isHidden: groupParams.hidden } })
        .commit();
    ...
    for (let i = 0; i < entities._rowCount; i++) {
        ...
        if (!entGroups.has(t)) {
            ...
            const group = await state.build()
                .to(entRoot)
                .applyOrUpdateTagged(`group:ent:${t}`, state_1.MesoscaleGroup, { ...groupParams, index: colorIdx, tag: `ent:${t}`, label: t, color: { type: 'generate', illustrative: false, value: color, variability: 20, shift: 0, lightness: 0, alpha: 1, emissive: 0 } }, { tags: `ent:`, state: { isCollapsed: true, isHidden: groupParams.hidden } })
    ...
    await state.transaction(async () => {
        try {
            ...
            for (let i = 0; i < entities._rowCount; i++) {
                ...
                build = build
                    .toRoot()
                    .apply(model_2.MmcifStructure, { structureRef: base.ref, entityId: entities.id.value(i) }, { dependsOn })
