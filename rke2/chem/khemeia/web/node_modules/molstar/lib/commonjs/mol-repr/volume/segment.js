...
const box3d_1 = require("../../mol-math/geometry/primitives/box3d.js");
...
function getLoci(volume, props) {
    const segments = sorted_array_1.SortedArray.ofUnsortedArray(props.segments);
    const instances = interval_1.Interval.ofLength(volume.instances.length);
    return volume_1.Volume.Segment.Loci(volume, [{ segments, instances }]);
}
function getSegmentLoci(pickingId, volume, key, props, id) {
    ...
    if (id === objectId) {
        ...
        if (granularity === 'volume') {
            ...
            const segments = ordered_set_1.OrderedSet.ofSingleton(key);
            return volume_1.Volume.Segment.Loci(volume, [{ segments, instances }]);
        }
        else {
...
function eachSegment(loci, volume, key, props, apply) {
    const segments = sorted_array_1.SortedArray.ofSingleton(key);
...
async function createVolumeSegmentMesh(ctx, volume, key, theme, props, mesh) {
    ...
    const instances = interval_1.Interval.ofLength(volume.instances.length);
    const segments = ordered_set_1.OrderedSet.ofSingleton(key);
    surface.setBoundingSphere(volume_1.Volume.Segment.getBoundingSphere(volume, [{ segments, instances }]));
    return surface;
}
...
function SegmentMeshVisual(materialId) {
    return (0, visual_1.VolumeVisual)({
        ...
        createLocationIterator: (volume, key) => {
            ...
            return (0, location_iterator_1.LocationIterator)(volume.grid.cells.data.length, volume.instances ? volume.instances.length : 1, 1, () => l);
        },
...
function getSegmentTexture(volume, segment, webgl) {
    ...
    const gridDataDim = linear_algebra_1.Vec3.clone(gridDimension);
...
async function createVolumeSegmentTextureMesh(ctx, volume, segment, theme, props, textureMesh) {
    ...
    const gv = (0, isosurface_1.extractIsosurface)(ctx.webgl, texture, gridDimension, gridTexDim, gridDataDim, gridTexScale, transform, 0.5, false, false, axisOrder, true, buffer === null || buffer === void 0 ? void 0 : buffer.vertex, buffer === null || buffer === void 0 ? void 0 : buffer.group, buffer === null || buffer === void 0 ? void 0 : buffer.normal);
    const groupCount = volume.grid.cells.data.length;
    const instances = interval_1.Interval.ofLength(volume.instances.length);
    const segments = ordered_set_1.OrderedSet.ofSingleton(segment);
    const boundingSphere = volume_1.Volume.Segment.getBoundingSphere(volume, [{ segments, instances }]);
...
//
function getSegments(props, _volume) {
    return sorted_array_1.SortedArray.ofUnsortedArray(props.segments);
}
