...
exports.TrackballControls = exports.DefaultTrackballControlsAttribs = exports.TrackballControlsParams = exports.DefaultTrackballBindings = void 0;
...
exports.DefaultTrackballBindings = {
    dragRotate: (0, binding_1.Binding)([
    ...
    keyMoveForward: (0, binding_1.Binding)([
        Key('KeyW'),
        Key('GamepadUp'),
    ], 'Move forward', 'Press ${triggers}'),
    keyMoveBack: (0, binding_1.Binding)([
        Key('KeyS'),
        Key('GamepadDown'),
...
exports.TrackballControlsParams = {
    ...
    animate: param_definition_1.ParamDefinition.MappedStatic('off', {
        ...
        spin: param_definition_1.ParamDefinition.Group({
            speed: param_definition_1.ParamDefinition.Numeric(0.1, { min: -2, max: 2, step: 0.01 }, { description: 'Number of rotations per second' }),
            axis: param_definition_1.ParamDefinition.Vec3(linear_algebra_1.Vec3.create(0, -1, 0), {}, { description: 'Axis of rotation in camera space' }),
        }, { description: 'Spin the 3D scene around an axis in camera space' }),
        rock: param_definition_1.ParamDefinition.Group({
            speed: param_definition_1.ParamDefinition.Numeric(0.3, { min: -5, max: 5, step: 0.1 }, { description: 'Number of oscilations per second' }),
...
exports.DefaultTrackballControlsAttribs = {
    bindings: exports.DefaultTrackballBindings,
};
var TrackballControls;
(function (TrackballControls) {
    function create(input, camera, scene, props = {}, attribs = {}) {
        const p = {
        ...
        function moveCamera(deltaT) {
            ...
            if (keyState.moveForward === 1) {
                ...
                if (cameraDistance < scene.boundingSphereVisible.radius && moveTarget) {
                    ...
                    linear_algebra_1.Vec3.sub(moveDir, camera.position, camera.target);
                    linear_algebra_1.Vec3.scale(moveDir, moveDir, 1 - moveSpeed / 100);
                    linear_algebra_1.Vec3.add(camera.position, camera.target, moveDir);
                }
                if (moveTarget) {
        ...
        function onDrag({ x, y, dx, dy, pageX, pageY, buttons, modifiers, isStart, useDelta }) {
            ...
            if (useDelta && dragRotate) {
                linear_algebra_1.Vec2.copy(_rotPrev, getMouseOnCircle(pageX - dx, pageY - dy));
            }
        ...
        const _animateQuat = (0, linear_algebra_1.Quat)();
        const _animateAxis = (0, linear_algebra_1.Vec3)();
        const _animateUp = (0, linear_algebra_1.Vec3)();
        const _animateSide = (0, linear_algebra_1.Vec3)();
        const _animateDir = (0, linear_algebra_1.Vec3)();
        function spin(deltaT) {
            ...
            linear_algebra_1.Vec3.sub(_eye, camera.position, camera.target);
            linear_algebra_1.Vec3.normalize(_animateDir, _eye); // Z-axis (view direction)
            linear_algebra_1.Vec3.normalize(_animateUp, camera.up); // Y-axis (up)
            linear_algebra_1.Vec3.cross(_animateSide, _animateUp, _animateDir); // X-axis (right)
            linear_algebra_1.Vec3.normalize(_animateSide, _animateSide);
            const axis = p.animate.params.axis;
            linear_algebra_1.Vec3.set(_animateAxis, axis[0] * _animateSide[0] + axis[1] * _animateUp[0] + axis[2] * _animateDir[0], axis[0] * _animateSide[1] + axis[1] * _animateUp[1] + axis[2] * _animateDir[1], axis[0] * _animateSide[2] + axis[1] * _animateUp[2] + axis[2] * _animateDir[2]);
            linear_algebra_1.Vec3.normalize(_animateAxis, _animateAxis);
            linear_algebra_1.Quat.setAxisAngle(_animateQuat, _animateAxis, angle);
            linear_algebra_1.Vec3.transformQuat(_eye, _eye, _animateQuat);
            linear_algebra_1.Vec3.transformQuat(camera.up, camera.up, _animateQuat);
            linear_algebra_1.Vec3.add(camera.position, camera.target, _eye);
        }
        let _rockPhase = 0;
        function rock(deltaT) {
            ...
            const maxAngle = (0, misc_1.degToRad)(p.animate.params.angle);
