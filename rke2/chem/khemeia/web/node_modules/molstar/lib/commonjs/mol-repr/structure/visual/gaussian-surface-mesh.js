...
async function createGaussianSurfaceMesh(ctx, unit, structure, theme, props, mesh) {
    ...
    const params = {
        isoLevel,
        scalarField: floodfill !== 'off' ? linear_algebra_1.Tensor.createFloodfilled(field, isoLevel, floodfill) : field,
        idField
    };
    ...
    const sphere = geometry_1.Sphere3D.expand((0, geometry_1.Sphere3D)(), unit.boundary.sphere, maxRadius + extraRadius);
...
function GaussianSurfaceMeshVisual(materialId) {
    return (0, units_visual_1.UnitsMeshVisual)({
        ...
        processValues: (values, geometry, props, theme, webgl) => {
            ...
            if (csp) {
                (0, color_smoothing_1.applyMeshColorSmoothing)(values, csp, webgl, colorTexture);
...
async function createStructureGaussianSurfaceMesh(ctx, structure, theme, props, mesh) {
    ...
    const extraRadius = radiusOffset * (1 + Math.exp(-smoothness));
    const sphere = geometry_1.Sphere3D.expand((0, geometry_1.Sphere3D)(), structure.boundary.sphere, maxRadius + extraRadius);
    surface.setBoundingSphere(sphere);
    return surface;
}
...
function createGaussianSurfaceTextureMesh(ctx, unit, structure, theme, props, textureMesh) {
    ...
    const create = (textureMesh) => {
        ...
        const boundingSphere = geometry_1.Sphere3D.expand((0, geometry_1.Sphere3D)(), unit.boundary.sphere, densityTextureData.maxRadius + extraRadius);
...
function GaussianSurfaceTextureMeshVisual(materialId) {
    return (0, units_visual_1.UnitsTextureMeshVisual)({
        ...
        processValues: (values, geometry, props, theme, webgl) => {
            ...
            const csp = (0, base_1.getColorSmoothingProps)(props.smoothColors, theme.color.preferSmoothing, resolution);
            if (csp && webgl) {
                (0, color_smoothing_2.applyTextureMeshColorSmoothing)(values, csp, webgl, colorTexture);
                geometry.meta.colorTexture = values.tColorGrid.ref.value;
            }
        },
        dispose: (geometry) => {
...
function createStructureGaussianSurfaceTextureMesh(ctx, structure, theme, props, textureMesh) {
    ...
    const create = (textureMesh) => {
        ...
        const extraRadius = props.radiusOffset * (1 + Math.exp(-props.smoothness));
        const boundingSphere = geometry_1.Sphere3D.expand((0, geometry_1.Sphere3D)(), structure.boundary.sphere, densityTextureData.maxRadius + extraRadius);
        const surface = texture_mesh_1.TextureMesh.create(gv.vertexCount, groupCount, gv.vertexTexture, gv.groupTexture, gv.normalTexture, boundingSphere, textureMesh);
        surface.meta.resolution = densityTextureData.resolution;
        return surface;
    };
