...
import { unpackRGBToInt, packIntToRGBArray } from '../../mol-util/number-packing.js';
export function createSizes(locationIt, positionIt, sizeTheme, sizeData) {
...
export function createVertexSize(locationIt, sizeFn, sizeData) {
    const { groupCount } = locationIt;
    const sizes = createTextureImage(Math.max(1, groupCount), 3, Uint8Array, sizeData && sizeData.tSize.ref.value.array);
    locationIt.reset();
    while (locationIt.hasNext) {
...
export function createVertexInstanceSize(locationIt, sizeFn, sizeData) {
    const { groupCount, instanceCount } = locationIt;
    const count = instanceCount * groupCount;
    const sizes = createTextureImage(Math.max(1, count), 3, Uint8Array, sizeData && sizeData.tSize.ref.value.array);
    locationIt.reset();
