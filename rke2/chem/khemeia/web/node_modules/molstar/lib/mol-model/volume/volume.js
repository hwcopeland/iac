...
import { CubeFormat } from '../../mol-model-formats/volume/cube.js';
import { EPSILON, equalEps } from '../../mol-math/linear-algebra/3d/common.js';
import { CustomProperties } from '../custom-property.js';
import { ParamDefinition as PD } from '../../mol-util/param-definition.js';
import { toPrecision } from '../../mol-util/number.js';
import { DscifFormat } from '../../mol-model-formats/volume/density-server.js';
import { mortonOrder3d } from '../../mol-data/util/hash-functions.js';
export var Volume;
(function (Volume) {
    function is(x) {
        var _a, _b, _c, _d;
    ...
    function areLociEqual(a, b) { return a.volume === b.volume && OrderedSet.areEqual(a.instances, b.instances); }
    Volume.areLociEqual = areLociEqual;
    function isLociEmpty(loci) { return isEmpty(loci.volume) || OrderedSet.isEmpty(loci.instances); }
    ...
    (function (Isosurface) {
        ...
        function areLociEqual(a, b) { return a.volume === b.volume && Volume.IsoValue.areSame(a.isoValue, b.isoValue, a.volume.grid.stats) && OrderedSet.areEqual(a.instances, b.instances); }
    ...
    (function (Cell) {
        ...
        function isLociEmpty(loci) {
            for (const { indices, instances } of loci.elements) {
                if (!OrderedSet.isEmpty(instances) || !OrderedSet.isEmpty(indices))
                    return false;
            }
            return true;
        }
    ...
    (function (Segment) {
        ...
        function isLociEmpty(loci) {
            for (const { segments, instances } of loci.elements) {
                if (!OrderedSet.isEmpty(instances) || !OrderedSet.isEmpty(segments))
                    return false;
            }
            return true;
        }
    ...
    function getPeriodicMapping(volume) {
        ...
        if (!v._localPropertyData['__periodicInstanceMapping__']) {
            ...
            for (let i = 0, il = v.instances.length; i < il; i++) {
                ...
                Vec3.round(tg, tg);
                Vec3.sub(tg, tg, min);
                map.set(mortonOrder3d(tg[0], tg[1], tg[2]), i);
            }
