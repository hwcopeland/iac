...
import { Database, Table } from '../../mol-data/db.js';
import { SetUtils } from '../../mol-util/set.js';
import { DefaultMap } from '../../mol-util/map.js';
import { mmCIF_chemCompBond_schema } from '../../mol-io/reader/cif/schema/mmcif-extras.js';
import { ccd_chemCompAtom_schema } from '../../mol-io/reader/cif/schema/ccd-extras.js';
import { DefaultDataOptions, ensureDataAvailable, getEncodedCif, readCCD, readPVCD } from './util.js';
...
function checkAddingBondsFromPVCD(pvcd) {
    const ccbSetByParent = DefaultMap(() => new Set());
    for (const k in pvcd) {
...
function checkAddingAtomsFromPVCD(pvcd) {
    const ccaSetByParent = DefaultMap(() => new Set());
    for (const k in pvcd) {
...
async function createBonds(ccd, pvcd, atomsRequested) {
    ...
    const bondDatabase = Database.ofTables(CCB_TABLE_NAME, { chem_comp_bond: mmCIF_chemCompBond_schema }, { chem_comp_bond: bondTable });
...
async function run(out, binary = false, options = DefaultDataOptions, ccaOut) {
