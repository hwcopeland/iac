...
import { addParamDefaults } from './params-schema.js';
import { getParams } from './tree-schema.js';
...
export function resolveUri(...refs) {
...
export function findUris(tree, uriParamNames, out = new Set()) {
    dfs(tree, node => {
        const params = node.params;
        if (!params)
            return;
...
export function replaceUris(tree, uriMapping, uriParamNames) {
    dfs(tree, node => {
        const params = node.params;
        if (!params)
            return;
