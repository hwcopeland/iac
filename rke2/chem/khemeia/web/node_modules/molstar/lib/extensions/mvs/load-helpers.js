 ...
 */
import { Mat3, Mat4, Quat, Vec3 } from '../../mol-math/linear-algebra.js';
import { Volume } from '../../mol-model/volume.js';
import { StructureInstances, TransformStructureConformation } from '../../mol-plugin-state/transforms/model.js';
import { VolumeInstances, VolumeTransform } from '../../mol-plugin-state/transforms/volume.js';
import { arrayDistinct } from '../../mol-util/array.js';
...
import { MultilayerColorThemeName, NoColor } from './components/multilayer-color-theme.js';
import { SelectorAll } from './components/selector.js';
import { MvsNamedColorDicts, MvsNamedColorLists } from './helpers/colors.js';
import { rowToExpression, rowsToExpression } from './helpers/selections.js';
import { decodeColor, isDefined, stringHash } from './helpers/utils.js';
import { mvsRefTags, UpdateTarget } from './load-generic.js';
import { getChildren } from './tree/generic/tree-schema.js';
import { dfs, formatObject } from './tree/generic/tree-utils.js';
import { DefaultColor } from './tree/mvs/mvs-tree.js';
import { CategoricalPaletteDefaults, ContinuousPaletteDefaults, DiscretePaletteDefaults } from './tree/mvs/param-types.js';
...
export function decomposeRotationMatrix(rotation) {
    if (rotation && rotation.length !== 9)
        throw new Error(`'rotation' param for 'transform' node must be array of 9 elements, found ${rotation}`);
    if (rotation) {
        const rotMatrix = Mat3.fromArray(Mat3(), rotation, 0);
...
const _tmpVecY = Vec3();
const _tmpVecZ = Vec3();
export function transformAndInstantiateStructure(target, node) {
    return applyTransformAndInstances(target, node, TransformStructureConformation, StructureInstances);
}
export function transformAndInstantiateVolume(target, node) {
...
function transformProps(node, kind) {
    const result = [];
    const transforms = getChildren(node).filter(c => c.kind === kind);
...
export function collectAnnotationReferences(tree, context) {
    ...
    dfs(tree, node => {
        ...
        if (spec) {
            ...
            (_a = distinctSpecs[key]) !== null && _a !== void 0 ? _a : (distinctSpecs[key] = { ...spec, id: stringHash(key) });
...
export function labelFromXProps(node, context) {
    ...
    const groupBy = (_a = node.params.group_by_fields) !== null && _a !== void 0 ? _a : [(_b = node.params.field_remapping.group_id) !== null && _b !== void 0 ? _b : 'group_id'];
    const nearestReprNode = (_c = context.nearestReprMap) === null || _c === void 0 ? void 0 : _c.get(node);
    return {
        type: { name: MVSAnnotationLabelRepresentationProvider.name, params: { annotationId, fieldName, textFormat, groupByFields: groupBy.map(x => ({ fieldName: x })) } },
...
function representationPropsBase(node) {
    ...
    switch (params.type) {
        ...
        case 'putty': {
            const sizeTheme = (_a = params.size_theme) !== null && _a !== void 0 ? _a : 'uniform';
            return {
...
export function clippingForNode(node) {
...
export function colorThemeForNode(node, context) {
    ...
    if ((node === null || node === void 0 ? void 0 : node.kind) === 'color_from_uri' || (node === null || node === void 0 ? void 0 : node.kind) === 'color_from_source') {
        ...
        return {
            ...
            params: { annotationId, fieldName, background: NoColor, palette: palettePropsFromMVSPalette(node.params.palette) },
        };
    }
}
...
export function palettePropsFromMVSPalette(palette) {
    var _a, _b, _c;
    if (!palette) {
        return { name: 'direct', params: {} };
    }
    if (palette.kind === 'categorical') {
        const fullParams = objMerge(CategoricalPaletteDefaults, palette);
        return {
            name: 'categorical',
            params: {
                ...
                missingColor: (_a = decodeColor(fullParams.missing_color)) !== null && _a !== void 0 ? _a : FALLBACK_COLOR,
            },
        };
    }
    if (palette.kind === 'discrete') {
        const fullParams = objMerge(DiscretePaletteDefaults, palette);
        return {
            name: 'discrete',
            params: {
    ...
    if (palette.kind === 'continuous') {
        const fullParams = objMerge(ContinuousPaletteDefaults, palette);
        ...
        return {
            ...
            params: {
                ...
                underflowColor: (_b = (fullParams.underflow_color === 'auto' ? minColor(colors.colors) : decodeColor(fullParams.underflow_color))) !== null && _b !== void 0 ? _b : FALLBACK_COLOR,
                setOverflowColor: !!fullParams.overflow_color,
                overflowColor: (_c = (fullParams.overflow_color === 'auto' ? maxColor(colors.colors) : decodeColor(fullParams.overflow_color))) !== null && _c !== void 0 ? _c : FALLBACK_COLOR,
            },
        };
    }
...
function categoricalPalettePropsFromMVSColors(colors) {
    if (typeof colors === 'string') {
        ...
        if (colors in MvsNamedColorDicts) {
            const colorDict = MvsNamedColorDicts[colors];
    ...
    if (Array.isArray(colors)) {
        return { name: 'list', params: { kind: 'set', colors: colors.map(c => { var _a; return (_a = decodeColor(c)) !== null && _a !== void 0 ? _a : FALLBACK_COLOR; }) } };
    }
    if (typeof colors === 'object') {
        return { name: 'dictionary', params: Object.entries(colors).map(([value, color]) => { var _a; return ({ value, color: (_a = decodeColor(color)) !== null && _a !== void 0 ? _a : FALLBACK_COLOR }); }) };
    }
...
function discretePalettePropsFromMVSColors(colors, reverse) {
    ...
    if (Array.isArray(colors) && colors.every(t => typeof t === 'string')) {
        ...
        return list.map((c, i) => { var _a; return ({ color: (_a = decodeColor(c)) !== null && _a !== void 0 ? _a : NoColor, fromValue: i * sectionLength, toValue: (i + 1) * sectionLength }); });
    }
    if (Array.isArray(colors) && colors.every(t => Array.isArray(t) && t.length === 2)) {
        return colors.map((t, i) => { var _a, _b, _c; return ({ color: (_a = decodeColor(t[0])) !== null && _a !== void 0 ? _a : NoColor, fromValue: t[1], toValue: (_c = (_b = colors[i + 1]) === null || _b === void 0 ? void 0 : _b[1]) !== null && _c !== void 0 ? _c : Infinity }); });
    }
    if (Array.isArray(colors) && colors.every(t => Array.isArray(t) && t.length === 3)) {
        return colors.map(t => { var _a, _b, _c; return ({ color: (_a = decodeColor(t[0])) !== null && _a !== void 0 ? _a : NoColor, fromValue: (_b = t[1]) !== null && _b !== void 0 ? _b : -Infinity, toValue: (_c = t[2]) !== null && _c !== void 0 ? _c : Infinity }); });
    }
    return [];
}
function continuousPalettePropsFromMVSColors(colors, reverse) {
    ...
    if (Array.isArray(colors)) {
        if (colors.every(t => Array.isArray(t))) {
            ...
            return { kind: 'interpolate', colors: colors.map(t => { var _a; return [(_a = decodeColor(t[0])) !== null && _a !== void 0 ? _a : FALLBACK_COLOR, t[1]]; }) };
        }
        else {
            // Color list without checkpoints
            const list = reverse ? colors.slice().reverse() : colors;
            const n = list.length - 1;
            return { kind: 'interpolate', colors: list.map((col, i) => { var _a; return [(_a = decodeColor(col)) !== null && _a !== void 0 ? _a : FALLBACK_COLOR, i / n]; }) };
        }
    }
...
function minColor(colors) {
    if (colors.length === 0)
        return undefined;
    if (colors.every(t => Array.isArray(t)))
        return Color.fromColorListEntry(colors.reduce((a, b) => a[1] < b[1] ? a : b));
