#!/usr/bin/env python3
"""AutoDock-GPU batch docking worker for v2 multi-engine pipeline.

Fetches receptor PDBQT from S3 (via target-prep ref), fetches ligand
conformer PDBQTs from S3 (via library-prep ref), runs AutoDock-GPU CLI
for each ligand, and writes results to docking_v2_results.
...
All configuration via environment variables:
  ...
  ENGINE         - engine identifier (always "autodock-gpu" for this script)
  ...
  EXHAUSTIVENESS - number of evaluations (default 32, maps to nev = exhaustiveness * 1M)
...
# AutoDock-GPU binary -- try multiple known locations
AUTODOCK_GPU = os.environ.get("AUTODOCK_GPU_BIN", None)
if AUTODOCK_GPU is None:
    for candidate in ["/usr/local/bin/autodock_gpu", "/usr/local/bin/autodock_gpu_128wi"]:
        if os.path.isfile(candidate) and os.access(candidate, os.X_OK):
            AUTODOCK_GPU = candidate
            break
if AUTODOCK_GPU is None:
    AUTODOCK_GPU = "autodock_gpu_128wi"  # fallback to PATH lookup
...
# Parse RANKING lines from DLG output:
#   "RANKING      1      -7.32      3.45"
_RANKING_RE = re.compile(r"RANKING\s+(\d+)\s+([-\d.]+)\s+([-\d.]+)")

# Fallback: parse energy from stdout
_ENERGY_RE = re.compile(r"([-]?\d+\.\d+)\s+kcal/mol")
...
def get_config():
    ...
    return {
        ...
        "engine": os.environ.get("ENGINE", "autodock-gpu"),
...
def parse_dlg(dlg_path, stdout_text):
    """Parse AutoDock-GPU DLG output for best ranked result.

    Returns (affinity, None) or (None, None) on failure.
    DLG does not contain PDBQT pose data in a readily extractable format.
    """
    # Try DLG file first
    if os.path.exists(dlg_path):
        try:
            with open(dlg_path) as f:
                dlg = f.read()
            matches = list(_RANKING_RE.finditer(dlg))
            if matches:
                # Best ranked result (rank 1)
                best = min(matches, key=lambda m: int(m.group(1)))
                return float(best.group(2)), None
        except Exception as exc:
            print(f"WARNING: failed to parse DLG: {exc}", flush=True)

    # Fallback: parse stdout for energy
    matches = list(_ENERGY_RE.finditer(stdout_text))
    if matches:
        energies = [float(m.group(1)) for m in matches]
        return min(energies), None

    return None, None


def run_autodock_gpu(receptor_path, ligand_pdbqt, exhaustiveness, n_poses=9):
    """Run AutoDock-GPU for a single ligand. Returns (affinity, docked_pdbqt) or (None, None)."""
    tmpdir = tempfile.mkdtemp(prefix="adgpu_")
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"WARNING: AutoDock-GPU failed (code {result.returncode}): {result.stderr[:300]}", flush=True)
            return None, None

        affinity, _ = parse_dlg(out_path, result.stdout)
        return affinity, None  # AutoDock-GPU DLG pose extraction is complex; omit for now

    except subprocess.TimeoutExpired:
        print("WARNING: AutoDock-GPU timed out (600s)", flush=True)
        return None, None
    finally:
        for f in [lig_path, out_path]:
            if os.path.exists(f):
                os.unlink(f)
        if os.path.isdir(tmpdir):
            try:
                ...
                pass  # May have leftover files


def main():
    cfg = get_config()
    print(
        f"AutoDock-GPU batch worker starting: job={cfg['job_name']} worker={cfg['worker_name']} "
    ...
    for i, (ligand_db_id, compound_id, pdbqt_bytes) in enumerate(ligands, start=1):
        try:
            affinity, docked_pdbqt = run_autodock_gpu(
            ...
            cursor.execute(
                ...
                "VALUES (%s, %s, %s, %s, %s, %s)",
                (cfg["job_name"], cfg["engine"], compound_id, ligand_db_id, affinity, None),
            )
            conn.commit()
            docked += 1
